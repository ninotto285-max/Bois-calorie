...
function parseOrario(t){
  ...
  const mappa = [
    [/\b6\s*e\s*mez/,'18:30'],[/\b6\s*e\s*trenta/,'18:30'],[/\b6\s*e\s*30\b/,'18:30'],[/\bsei\s*e\s*mez/i,'18:30'],[/\balle\s*6\s*e\s*mez/,'18:30'],
    [/\b6\s*e\s*un\s*quarto/,'18:45'],[/\balle?\s*sei\b/,'18:30'],
    [/\b7\s*e\s*mez/,'19:30'],[/\b7\s*e\s*trenta/,'19:30'],[/\b7\s*e\s*30\b/,'19:30'],[/\bsette\s*e\s*mez/i,'19:30'],
    [/\b7\s*e\s*un\s*quarto/,'19:15'],[/\balle?\s*sette\b/,'19:00'],[/\balle?\s*7\b/,'19:00'],
    [/\b8\s*e\s*mez/,'20:30'],[/\b8\s*e\s*trenta/,'20:30'],[/\b8\s*e\s*30\b/,'20:30'],[/\botto\s*e\s*mez/i,'20:30'],
    [/\b8\s*e\s*un\s*quarto/,'20:15'],[/\balle?\s*otto\b/,'20:00'],[/\balle?\s*8\b/,'20:00'],
    [/\b9\s*e\s*mez/,'21:30'],[/\b9\s*e\s*trenta/,'21:30'],[/\b9\s*e\s*30\b/,'21:30'],[/\bnove\s*e\s*mez/i,'21:30'],
    [/\b9\s*e\s*un\s*quarto/,'21:15'],[/\balle?\s*nove\b/,'21:00'],[/\balle?\s*9\b/,'21:00'],
    [/\b18\s*e\s*mez/,'18:30'],[/\b19\s*e\s*mez/,'19:30'],[/\b20\s*e\s*mez/,'20:30'],[/\b21\s*e\s*mez/,'21:30'],
    [/\bper\s*le\s*19\b/,'19:00'],[/\bper\s*le\s*20\b/,'20:00'],[/\bper\s*le\s*21\b/,'21:00'],[/\bper\s*le\s*18\b/,'18:30'],
    [/\balle?\s*6\b/,'18:30'],[/\bverso\s*(?:le?)?\s*8\b/i,'20:00'],[/\bverso\s*(?:le?)?\s*7\b/i,'19:00'],
    [/\bverso\s*(?:le?)?\s*9\b/i,'21:00'],[/\bverso\s*(?:le?)?\s*6\b/i,'18:30'],
    [/^18$/,'18:30'],[/^19$/,'19:00'],[/^20$/,'20:00'],[/^21$/,'21:00'],
  ];
  for(const [re, orario] of mappa){ if(re.test(t)) return orario; }
  return null;
}
...
function fmtOrdine(){
  ...
  for(const p of ordine.pizze){ msg += `• ${p.qty}x **${p.nome}** — ${fmtE(p.prezzo * p.qty)}€\n`; }
  if(ordine.frittini && ordine.frittini.length){ for(const f of ordine.frittini) msg += `• ${f.qty}x Fritino ${f.tipo} 5pz — ${fmtE(f.prezzo*f.qty)}€\n`; }
  if(ordine.bibite && ordine.bibite.length){ for(const b of ordine.bibite) msg += `• ${b.qty}x ${b.label} — ${fmtE(b.prezzo*b.qty)}€\n`; }
  ...
  if(ordine._noteVariazioni && ordine._noteVariazioni.length) msg += '\n\n📌 *Richieste speciali:*\n'+ordine._noteVariazioni.join('\n');
  if(ordine.note) msg += '\n📝 Note: '+ordine.note;
  return msg;
}
...
async function inviaOrdine(){
  ...
  try {
    const r = await fetch('/api/ordine', {
      ...
      body: JSON.stringify({ nome:ordine.nome, telefono:ordine.telefono||'', orario:ordine.orario,
        pizze:ordine.pizze.map(p=>({qty:p.qty,nome:p.nome,prezzo:p.prezzo})), totale:tot,
        note:ordine.note, frittini:ordine.frittini||[], noteVariazioni:ordine._noteVariazioni||[], bibite:ordine.bibite||[] })
    });
...
function gestisciOrdine(input){
  ...
  if(ordineStep === 'pizze'){
    if(['basta','ok basta','ho finito','fine','finito'].some(k=>norm(input.trim())===k)){
    ...
    let _inp = correggiTypo(input)
      .replace(/\b4\s+formaggi\b/gi,'formaggi').replace(/\b4stagioni\b/gi,'quattro stagioni').replace(/\b4\s+stagioni\b/gi,'quattro stagioni')
    ...
    const righe = _inp.split(/,|\n|(?<=\S)\s+e\s+(?=\d)|(?<=\S)\s+e\s+(?=una?\s)|(?<=\S)\s+e\s+(?=un[ao]?\s)/).filter(r=>r.trim());
    let trovate = [];
    for(let riga of righe){
      riga = riga.replace(/-e-/g,' e ');
      const rn = norm(riga);
      ...
      const senzaMatch = riga.match(/\bsenza\s+([\w\s]+?)(?=\s*,|\s+e\s+[a-z]|\s+con\s+|$)/i);
      const ingredienteSenza = senzaMatch ? senzaMatch[1].trim() : null;
      const rigaSenzaNum = riga.replace(/\d+/g,'').trim();
      const rigaPerNome = rigaSenzaNum.replace(/\bsenza\s+[\w\s]+?(?=\s+(?:con\b|doppi|battut|baby)|$)/gi,'').trim();
      const tentativi = [rigaPerNome, rigaSenzaNum, rigaSenzaNum.replace(/e$/,'a'), rigaSenzaNum.replace(/he$/,'ha'), rigaSenzaNum.replace(/i$/,'a'), rigaSenzaNum.replace(/ie$/,'ia'), rn.replace(/\d+\s*/g,''), riga];
      let nomePizza = null;
  ...
  if(ordineStep === 'altra'){
    if(['basta','ok','no','finito','fatto','va bene','è tutto','e tutto'].some(k=>t.includes(k))){
      ordineStep = 'note';
      return `Hai note particolari? (allergie, variazioni, altro)\nOppure scrivi **"no"** per procedere al riepilogo.`;
    }
    ordineStep = 'pizze';
  ...
  if(ordineStep === 'note'){
    ...
    if(['no','niente','nessuna','nessuno','nope','nah'].some(k=>tN===k)){ ordine.note = ''; }
    else { ordine.note = input.trim(); }
    ordineStep = 'spicchi';
  ...
  if(ordineStep === 'spicchi'){
    ...
    if(['si','sì','ok','yes','certo','dai'].some(k=>tS.includes(k))){
  ...
  if(ordineStep === 'frittini'){
    ...
    if(['no','niente','nope','nah','no grazie','non voglio'].some(k=>tF===k)){
      ordineStep = 'bibita';
      return 'Vuoi aggiungere anche una **bibita**? 🥤\n• Analcolica (Coca, Fanta, Sprite) — 2,50€\n• Birra lattina — 2,50€\n• Birra bottiglia — 3,50€\nOppure **"no"** per il riepilogo.';
    }
    const nF = input.match(/(\d+|uno|due|tre|quattro|cinque)/i);
    const numFritt = nF ? ({'uno':1,'due':2,'tre':3,'quattro':4,'cinque':5}[nF[1].toLowerCase()]||parseInt(nF[1])||1) : 1;
    let tipoF = 'Misti';
    if(!tF.includes('mist')&&!tF.includes('assort')){
      const paroleF = tF.split(/\s+/);
      for(const nome of Object.keys(FRITTINI)){
        if(paroleF.some(pw=>norm(nome).includes(pw)&&pw.length>3)){ tipoF=nome.charAt(0).toUpperCase()+nome.slice(1); break; }
      }
    }
  ...
  if(ordineStep === 'bibita'){
    ...
    if(keyBibita && BIBITE[keyBibita]){
      ...
      ordine.bibite.push({ qty:1, label:bib.label, prezzo:bib.prezzo });
      ordineStep = 'conferma';
      return 'Aggiunto: 1x '+bib.label+' — '+fmtE(bib.prezzo)+'€ 🥤\n\n'+fmtOrdine()+'\n\nÈ tutto corretto?';
    }
    if(['si','sì','ok','yes','certo','dai'].some(k=>tB===k)){
      return 'Quale bibita vuoi? 🥤\n• Bibita analcolica — 2,50€\n• Birra lattina — 2,50€\n• Birra bottiglia 33/50cl — 3,50€\n• Tuborg 66cl — 3,50€\n• Birra da litro — 6,00€';
    }
    ordineStep = 'conferma';
    return fmtOrdine() + '\n\nÈ tutto corretto?';
  }
  ...
  if(ordineStep === 'conferma'){
    if(['si','sì','ok','confermo','giusto','esatto','corretto','vai'].some(k=>t.includes(k))){ return 'MOSTRA_PULSANTE'; }
    if(['no','sbagliato','annulla'].some(k=>t.includes(k))){ resetOrdine(); return 'Ordine annullato. Ricominciamo? Scrivi "voglio ordinare"! 🐧'; }
...
async function bpSend(){
  ...
  if(ordineAttivo){
    // Rispondi prima a domande locali anche durante l'ordine
    if(ordineStep !== 'nome' && ordineStep !== 'telefono'){
      const localeOT = rispostaLocale(text);
      if(localeOT && localeOT !== 'ORDER' && localeOT !== 'COSA_SAI_FARE'){
        const stepCorrente = ordineStep;
        const domanda = stepCorrente==='orario'?'a che ora vieni?':stepCorrente==='pizze'?'quali pizze vuoi?':stepCorrente==='altra'?'vuoi altro o scrivi "basta"?':'confermi?';
        setTimeout(()=>addBotMsg(localeOT+'\n\n🐧 Riprendiamo l\'ordine — '+domanda),300);
        return;
      }
    }
  ...
  // ── TRIGGER ORDINE — FIX: "no voglio" non deve triggerare ──
  const tn = norm(text);
  const triggerOrdine = [
    'voglio ordinare','vorrei ordinare','posso ordinare',
    'fare un ordine','faccio un ordine','mando un ordine','faccio ordine',
    'voglio prenotare','vorrei prenotare','prenota per',
    'ordiniamo stasera','ordiniamo le pizze'
  ];
  const isNegazione = tn.startsWith('no ') || tn.startsWith('non ') || tn.includes('non voglio');
  if(!isNegazione && triggerOrdine.some(k=>tn.includes(norm(k)))){
    ordineAttivo = true;
    const orarioGia = parseOrario(tn);
    if(orarioGia){
...
function mostraCuriosita(){
  ...
  setTimeout(()=>addBotMsg('🐧 Lo sapevi?\n\n' + getCuriosita()), panelOpen ? 0 : 350);
}
